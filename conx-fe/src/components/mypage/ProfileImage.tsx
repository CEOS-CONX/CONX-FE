...
interface ProfileImageProps {
  src?: string;
  /** 기본 이미지 결정용 seed (엔티티 id) — src 없을 때 4종 중 고정 배정 */
  seed?: number | string;
  alt?: string;
  /** 파일 선택 시 */
...
export default function ProfileImage({
  ...
  const uploaded = !!src; // 사용자가 직접 올린 이미지 여부 (삭제 버튼·오버레이 구분용)
  const displaySrc = src || defaultProfileImage(seed); // 없으면 seed 기반 기본 이미지
  const overlay = uploaded
  ...
  return (
    <div className="group flex items-center gap-1.5">
      ...
      <button
        type="button"
        aria-label={uploaded ? '이미지 변경' : '이미지 추가'}
        onClick={onEdit ?? open}
